...
{ 
    ...
    result = eglGetConfigs(display, configs.data(), numConfigs, &numConfigs);
    for (int i = 0; i < numConfigs; i ++) {
        printf("Config #%d:\n", i);
        EGLint buffer_size;
        eglGetConfigAttrib(display, configs[i], EGL_BUFFER_SIZE, &buffer_size);
        ...
        printf(" Red, Green, Blue, Alpha sizes: %d, %d, %d, %d\n", red_size, green_size, blue_size, alpha_size);
        EGLint caveat;
        eglGetConfigAttrib(display,configs[i],EGL_CONFIG_CAVEAT, &caveat);
        switch(caveat)
        {
            case  EGL_NONE        : printf(" Caveat: none\n"); break;
            case  EGL_SLOW_CONFIG : printf(" Caveat: slow config\n"); break;
        }
        EGLint max_pbuffer_width, max_pbuffer_height;
        eglGetConfigAttrib(display,configs[i], EGL_MAX_PBUFFER_WIDTH , &max_pbuffer_width );
        eglGetConfigAttrib(display,configs[i], EGL_MAX_PBUFFER_HEIGHT, &max_pbuffer_height);
        printf(" Max pbuffer width x height: %d x %d\n", max_pbuffer_width, max_pbuffer_height);
        EGLint native_renderable;
        eglGetConfigAttrib(display,configs[i], EGL_NATIVE_RENDERABLE, &native_renderable);
        printf(" Native-renderable: %s\n", native_renderable ? "true" : "false");
        EGLint native_visual_id;
        eglGetConfigAttrib(display,configs[i], EGL_NATIVE_VISUAL_ID, &native_visual_id);
        printf(" Native visual ID: %d\n", native_visual_id);
        EGLint native_visual_type;
        eglGetConfigAttrib(display,configs[i], EGL_NATIVE_VISUAL_TYPE, &native_visual_type);
